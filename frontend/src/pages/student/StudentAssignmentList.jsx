// src/pages/student/StudentAssignmentList.jsx (FIXED MEMOIZATION)

import React, { useEffect, useState, useCallback, useMemo } from 'react'; // <-- Imported useMemo
...
const StudentAssignmentList = () => {
    ...
    // 2. MEMOIZE THE DERIVED STATE (FIX)
    const { assignments, pagination, isLoading } = useMemo(() => {
        // Safely access the nested object for this classId
    ...
    }, [assignmentsBaseState, classId]); // Only recalculate if the base state or classId changes
    ...
    useEffect(() => {
        dispatch(fetchAssignments({ 
            ...
            statusFilter: '' // Student view typically defaults to all active
        }));
    }, [dispatch, classId, currentPage, debouncedSearchTerm]);
    ...
    const getStatusBadge = (dueAt) => {
        ...
        return (
            // KEY CHANGE 1: Ensured badge is small and compact
    ...
    const AssignmentTable = ({ assignments }) => (
        // The overflow-x-auto is already correct for responsiveness
                    ...
                    {assignments.map((assignment) => (
                        <tr key={assignment._id} className="hover:bg-[#2b2b2b] transition-colors">
                            ...
                            <td className="px-4 py-3 whitespace-nowrap text-xs sm:text-sm text-[#bdbdbd]">
                                {/* KEY CHANGE 4: Use a shorter date format for mobile screens */}
                            ...
                            <td className="px-4 py-3 whitespace-nowrap text-right text-sm font-medium">
                                ...
                                >
                                    <Download className="h-3 w-3 sm:h-4 sm:w-4 mr-1 flex-shrink-0"/> View/Submit
                                </Link>
                            </td>
                        </tr>
                    ))}
    ...
    return (
        <div className="space-y-8 sm:space-y-10">
            {/* HEADER SECTION - KEY CHANGE 5: Reduced header text size for mobile */}
