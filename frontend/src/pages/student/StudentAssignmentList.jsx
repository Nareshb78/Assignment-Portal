// src/pages/student/StudentAssignmentList.jsx
import React, { useEffect, useState, useCallback, useMemo } from 'react'; 
import { useDispatch, useSelector } from 'react-redux';
import { useParams, Link } from 'react-router-dom';
import { fetchAssignments } from '../../redux/slices/assignmentSlice';
import { Search, Filter, ListOrdered, CheckCircle, XCircle, Download, ChevronDown } from 'lucide-react';
    ...
    <div className="overflow-x-auto bg-surface rounded-xl shadow-lg border border-[#2f2f2f]">
      <table className="min-w-full divide-y divide-gray-700">
        <thead className="bg-[#2b2b2b]">
          <tr>
            <th className="px-4 py-3 text-left text-xs font-semibold text-[#ba68c8] uppercase tracking-wider">Assignment</th>
            <th className="px-4 py-3 text-left text-xs font-semibold text-[#ba68c8] uppercase tracking-wider">Due Date</th>
            <th className="px-4 py-3 text-left text-xs font-semibold text-[#ba68c8] uppercase tracking-wider">Status</th>
            <th className="px-4 py-3 text-right text-xs font-semibold text-[#ba68c8] uppercase tracking-wider">Action</th>
          </tr>
        </thead>
        <tbody className="divide-y divide-gray-800">
          {assignments.map((assignment) => (
            <tr key={assignment._id} className="hover:bg-[#2b2b2b] transition-colors">
              <td className="px-4 py-3 text-sm font-medium text-[#e0e0e0] max-w-[200px] truncate">{assignment.title}</td>
              <td className="px-4 py-3 whitespace-nowrap text-xs sm:text-sm text-[#bdbdbd]">
                {new Date(assignment.dueAt).toLocaleDateString()}
              </td>
              <td className="px-4 py-3 whitespace-nowrap">{getStatusBadge(assignment.dueAt)}</td>
    ...
    <div className="space-y-8 sm:space-y-10">
      ...
      <header className="pb-4 border-b border-gray-700">
        <h1 className="text-3xl sm:text-4xl font-extrabold text-[#e0e0e0] tracking-wider">
          Assignments for <span className="text-[#03DAC6]">FSD 101 - Your Class</span>
        </h1>
        <p className="text-[#bdbdbd] mt-1 text-sm sm:text-lg flex items-center">
          <ListOrdered className="h-4 w-4 sm:h-5 sm:w-5 mr-2 text-[#ba68c8]" /> Browse available assignments.
        </p>
      </header>
